    ...
    <div class="container mx-auto max-w-4xl py-8 px-6 contain-content">
        <div class="bg-white shadow-md rounded-lg mb-6 p-6">
            ...
            <div>
                <div class="flex items-center justify-between p-0">
                    ...
                    @role('admin')
                        <form action="{{ route('history.destroy', $history->id) }}"
                            onsubmit="return confirm('Hujjatni rostanham o\'chirmoqchimisiz?')" method="POST"
                            class="mt-2">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                ...
                @if ($history->file)
                    <div class="mt-4">
                        <a href="{{ route('downloadFile', $history->id) }}" class="text-blue-600 hover:underline">
                            To'liq hujjat faylni yuklab olish
                        </a>
                    </div>
                @endif

                <div class="mt-8">
                    <h3 class="text-xl font-semibold text-gray-800 mb-4">Izohlar:</h3>

                    @foreach ($history->comments()->latest()->get() as $comment)
                        <div class="bg-gray-100 p-4 rounded-lg mb-4 flex justify-between">
                            ...
                            @if (auth()->id() === $comment->user_id || auth()->user()->hasRole('admin'))
                                <form action="{{ route('comment.destroy', $comment->id) }}" method="POST"
                                    class="inline-block"
                                    onsubmit="return confirm('Izohni rostanham o\'chirmoqchimisiz?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <form action="{{ route('comment.store', $history->id) }}" method="POST">
                        @csrf
                        <input type="hidden" name="history_id" value="{{ $history->id }}">
                        <textarea name="content" rows="4" class="w-full p-4 border border-gray-300 rounded-lg"
